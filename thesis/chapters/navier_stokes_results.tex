...
We now verify that (\ref{eq:ns_sol_2_r},b) is indeed an exact solution. Firstly,
we have that
\begin{equation*}
\frac{\partial u_i}{\partial z_j} = \alpha_1 \delta_{ij}
+ \alpha_2\charfcn{\Omega_+(t)}\bigg(\delta_{ij}\big(|\vec z\,|^2-r(t)^2\big)
+ 2z_iz_j\bigg)\,,
\end{equation*}
and
\begin{equation*}
\frac{\partial^2 u_i}{\partial z_j^2} =
2 \alpha_2\charfcn{\Omega_+(t)}\big(\delta_{ij} z_j+ z_i\big)\,.
\end{equation*}
...
= 2 \alpha_2\charfcn{\Omega_+(t)}\sum_{j=1}^d\big(\delta_{ij} z_j + z_i \big)
= 2(d+1)\alpha_2\charfcn{\Omega_+(t)}z_i\,.
\end{align*}
