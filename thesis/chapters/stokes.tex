...
Finally, as in the 2d case, see Table~\ref{tab:shear_3d_preconditioners}, we
...
(\ref{eq:SchurkX}) at the first time step, $m=0$. We employ uniform bulk meshes
and the P2--P0 element. We test the direct preconditioner (\ref{eq:direct})
factorizing it alternatively with either UMFPACK or SPQR and the block
preconditioner (\ref{eq:ESW}) with restart value either 50 or 1000.
\begin{table*}
\center
\hspace*{-3.25cm}
\begin{tabular}{rrrrrrrrr}
...
{($\mu=1,\gamma=3$) Time and number of GMRES iterations to solve the algebraic
system (\ref{eq:SchurkX}) adopting different preconditioners for the 3d
shear flow using P2--P0 element, uniform mesh and $m=0$.}
\label{tab:shear_3d_preconditioners}
\end{table*}
We observe that the 3d performance results are consistent with the 2d results.