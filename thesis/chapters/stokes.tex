...
(\ref{eq:constcurv}). See \S\ref{sec:45} for details.
...
\end{pmatrix} \,.
\end{equation}
Then (\ref{eq:stokes_algebraic}) can be reduced to
\begin{subequations}
...
We note that the interface condensed operator (\ref{eq:Xi}) is
exactly the operator (\ref{eq:algebraic_initial_curvature}) which is the
discretization of the stationary geometric PDE.

With a view towards some numerical test cases in \S\ref{sec:4}, we also
...
example, in place of (\ref{eq:HGb}), with $\pnormspace^m$ replaced by
$\pspace^m$, we require
...
(\ref{eq:HGa},c,d), (\ref{eq:LAb}) is exactly the same as the proof of
Theorem~\ref{thm:ex}.
