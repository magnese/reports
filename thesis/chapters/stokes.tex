...
\label{eq:stokes_full_initial_velocity} \\
...
(\ref{eq:stokes_full_momentum}--g) is given as follows. Given $\Gamma(0) =
\Gamma_0$,
...
$p \in \pspace$ is part of a solution to (\ref{eq:stokes_full_momentum}--g),
...
conservation property for the system (\ref{eq:stokes_weaka}--d). For the former,
we use (\ref{eq:length_variation}) and we obtain
\begin{equation}\label{eq:dtarea}
\ddt\,\surfvol(\Gamma(t)) = -
\left\langle \varkappa,\V\,.\,\vec\nu\right\rangle_{\Gamma(t)}.
\end{equation}
Hence, on choosing $\vec\xi = \vec u$ in (\ref{eq:stokes_weaka}), and noting
...
\end{equation}
\end{subequations}
is an exact solution to the problem (\ref{eq:stokes_full_momentum}--g) on e.g.\
...
\end{equation}
\end{subequations}
is an exact solution to the problem (\ref{eq:stokes_full_momentum}--g) with the
homogeneous right hand side in (\ref{eq:stokes_full_initial_velocity}) replaced
by
