...
test function and performing integration by parts (\ref{eq:integration_by_part})
yielding to
...
\begin{equation}\label{eq:weak_stress_jump}
\begin{align*}
...
compatibility condition $\int_{\partial_1\Omega}\vec g\,.\,\unitn=0$ for a
solution to (\ref{eq:stokes_weaka}--d) to exist.
...
means that $\vec U(\vec z)=\vec z$ for every corner $\vec z$ of $\Omega^m$.
Therefore $\vec U(\vec z)=\vec 0$ for all $\vec z\in\Omega^m$. In addition, it
...
\end{theorem}
\begin{proof}
As $\uspacesimple^m_0(\vec g)$ is an affine subspace of $\uspacedisc{g}{m}$,
...
\mat D(\vec U^{m+1}) \right) \leq \\
& \qquad \gamma\,\surfvol(\Gamma^m) + \tau_m\left( \vec f^{m+1}, \vec U^{m+1}
\right).
\end{align*}
\end{equation}
...
\mat D(\vec U^{k+1}), \mat D(\vec U^{k+1}) \right)\leq \\
& \qquad \gamma\,\surfvol(\Gamma^0) + \sum_{k=0}^m \tau_k\left(\vec f^{k+1},
\vec U^{k+1} \right)
\end{align*}
\end{equation}
for $m=0,\ldots, M-1$.
...
$\mat D(\vec U^{m+1}) = \mat 0$. Therefore, as shown in Theorem~\ref{thm:ex},
it implies $\vec U^{m+1} = \vec 0$.
\end{proof}
...
\frac{\left(\varphi, 1\right)}{\vol(\Omega)}\, \int_{\partial_1\Omega}
(\vec I^m_2\,\vec g) \,.\, \unitn \dH{d-1}
\quad \forall\ \varphi \in \pnormspace^m\,,\label{eq:HGcorrectedb} \\
&  \left\langle \frac{\vec X^{m+1} - \vec\id}{\tau_m} ,\chi\,\vec\nu^m
\right\rangle_{\Gamma^m}^h - \left\langle \vec U^{m+1}, \chi\,\vec\nu^m
\right\rangle_{\Gamma^m}  = 0 \quad \forall\ \chi \in \Wh\,,
...
$\Wh$. Then, ignoring for now the Dirichlet and free-slip boundary condition,
...
aforementioned basis functions. Moreover, $\vec X^m$ denotes the coefficients of
...
boundary degree of freedoms in $\vec B_\Omega$, $\vec C_\Omega$ and $\Nbulk$ in
...
condition is enforced doctoring the matrix entries corresponding to the degree
of freedoms of the basis functions $\phi_i^{\uspacesimple^m}\vec e_q$ defined on
part of $\partial_2\Omega$ which satisfy $\vec e_q=\unitn$. Instead, the
right-hand side is left unchanged.
...
using a matrix doctoring which consists on zeroing, for example, the first
row of the second block row of (\ref{eq:stokes_direct_precond}) and setting an
unitary value on the first diagonal entry in the null matrix. This doctoring
...
functions support is $\Omega_-$ and $\Omega_+$, respectively.
