...
\pspace^m\,.
\end{equation}
Therefore, when the boundary integral of the quadratic interpolation of the
boundary data $\vec g$ is different from zero and the pressure space used is
the unconstrained space $\pspace^m$ instead of $\pnormspace^m$ then the
equation (\ref{eq:LAb}) replaces (\ref{eq:HGb}). The proof of existence of an
unique solution to the inhomogeneous system (\ref{eq:HGa},c,d), (\ref{eq:LAb})
is exactly the same as the proof of Theorem~\ref{thm:ex}.
