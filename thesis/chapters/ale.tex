...
by setting
\footnote{Marco: I have made changes here.}
\begin{align*}
\uspaceale b &:= \{ \vec\phi:
...
approximation of (\ref{eq:ns_weaka_ale}--d). Let ${\cal
T}^h$ be a regular partitioning of the reference domain $\D$ into disjoint
open simplices $\sigmaO^h_j$, $j = 1 ,\ldots, J^h_\D$. From now on, the
reference domain $\D$, which we consider, is the polyhedral domain defined by 
the triangulation ${\cal T}^h$. 
\footnote{Marco: There is no time dependence on reference manifold, so I have
removed it starting from here. You need to sort out some macros.}
On ${\cal T}^h$ we define the finite element spaces
\begin{equation*}
