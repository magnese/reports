...
%%% The below is incorrect, as the continuous spaces are space-time, while for
%%% the discrete spaces we only need space. As we do not refer to LBB in ALE
%%% chapter, I have removed the paragraph.
...
\footnote{Marco: You need to do the $s+1$ change as, as I have done in previous
chapter. That is because e.g. $P^{m+1}$ changes at every iteration, so you
cannot use the same symbol for it.}
\begin{subequations}
\begin{align}
