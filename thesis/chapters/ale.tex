...
,\ldots, M-1$. It is now straightforward to obtain a full finite element
...
for $\vec U^{m+1}$ and $\W^{m+1}$ in the term $\vec D^h_\Omega\big(t^{m+1},\vec
...
(\ref{eq:ns_HGimpa}--d). More precisely, using a fixed point iteration, we can
reformulate the scheme (\ref{eq:ale_HGa}--d) as follow. Let $\Gamma^0$, an
approximation to $\Gamma(0)$, and $\vec U^0\in \uspacediscale{g}{0}$ be given.
For $m=0,\ldots, M-1$, set $\vec U^{m+1,0}=\vec U^m$, $\vec \psi^{m+1,0} =
\vec 0$, $\Omega^{m+1,0}=\Omega^m$, $s=0$ and $\epsilon_f \geq 0$.
Then, find $(\vec U^{m+1,s+1},P^{m+1}, \vec X^{m+1}, \kappa^{m+1}) \in
...
and $\|\vec \psi^{m+1,s+1}-\vec \psi^{m+1,s}\|_{L^\infty} \leq\epsilon_f$. The
...
$\vec U^{m+1}= \vec U^{m+1,s+1}$. The scheme (\ref{eq:ns_HGalea}--d), at each
...
\leq\epsilon_f$ is smaller than the required tolerance $\epsilon_f$. The
