...
denote the $L^2$--norm and the $H^1$--norm on $\D$, respectively.

Then, using the ALE mapping, we can define define the finite element spaces on
the moving domain $\Omega(t)$
\begin{align*}
\uspacesemidiscale{g}{h}{t} &:= \{ \vec\phi:\Omega(t)\times [0,T]\to \R^d,\quad
\vec \phi=\vec{\hat\phi}\circ\vec x^{\,-1},\quad\vec{\hat\phi}
\in\uspacesemidiscref{g}{h}{t}\}\,,\\
\pnormspaceale^h(t) &:= \{ \eta:\Omega(t)\times [0,T]\to \R,\quad
\eta=\hat\eta\circ\vec x^{\,-1},\quad\hat\eta\in\pnormspaceref^h(t) \}\,.
\end{align*}
