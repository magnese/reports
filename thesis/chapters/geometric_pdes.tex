...
mesh properties, see \S\ref{sec:geometric_pdes_semi_fem}. For stability it is
...
\vec\id\!\mid_{\Gamma^m}$ in (\ref{eq:fem_mean_curvature_b}), summing the
...
\geq \surfvol(\Gamma^{m+1}) - \surfvol(\Gamma^m)\,,
\end{equation*}
and, summing for $m=0,\ldots,k-1$, we finally obtain
\begin{equation*}
\surfvol(\Gamma^k)+\sum_{m=0}^{k-1} \tau_m(|k^{m+1}|_{\Gamma^m}^h)^2 \leq
\surfvol(\Gamma^0)\,,
\end{equation*}
...
aforementioned finite element schemes. For example, the finite element
approximation of the surface diffusion problem (\ref{eq:fem_surf_diff_a}--b)
can be replaced by the following one. Given $\Gamma^h(0)$, for $t\in (0,T]$
find $\{\vec X^h, \kappa^h \} \in \Vht \times \Wht$ such that
\begin{subequations}
\begin{align}
\langle \vec X^h_t, \chi \, \vec \nu^h \rangle_{\Gamma^h(t)}^h -
...
$\vec \nu^h(t)$, described by the identity function $\vec X^h(t) \in \Vht$. In
...
where $\Gamma^m$ and $\vec X^m$ are replaced by $\Gamma^h(t)$ and
$\vec X^h(t)$, respectively.
...
0 = \langle \vec X^h_t, \vec \nu^h \rangle^h_{\Gamma^h(t)}
= \int_{\Gamma^h(t)} \vec X^h_t \, \cdot \, \vec \nu^h \dH{d-1}
= \frac {d}{dt} \, \vol(\Omega^h(t)) \,,
\end{equation}
...
sequentially ordered vertices on $\Gamma^h(t)$. Let $\{ \vec X^h, \kappa^h \}$
