...
a parametrization $\vec{X}^{m+1} \in \Vh$, so that $\Gamma^{m+1} =
\vec{X}^{m+1}(\Gamma^m)$.
...
preserved, and that the volume loss tends to zero as $\tau \rightarrow 0$.
...
\begin{equation}
r(t) = \sqrt{r_0^2 - 2(d - 1)t}\,, \quad t\in \big[0,t_e\big)\,,
\end{equation}
...
$\Gamma(t)$ shrinks to a point as $t \rightarrow t_e$.
