...
\nonumber \\
& \qquad = \left(\vec f^{m+1},\vec \xi\right) \quad \forall\ \vec\xi \in
\uspacesimple^m_0(\vec 0)\,, \label{eq:ns_reda}\\
&  \left\langle \frac{\vec X^{m+1} - \vec\id}{\tau} ,\chi\,\vec\nu^m
\right\rangle_{\Gamma^m}^h - \left\langle \vec U^{m+1}, \chi\,\vec\nu^m
\right\rangle_{\Gamma^m}  = 0 \quad \forall\ \chi \in \Wh\,,
\label{eq:ns_redb} \\
...
unconditional stable. The proof is analogous to the Stokes case presented in
\S\ref{sec:stokes_stability}.
...
\vec I^m_2\,\vec U^m \right) + \tau\left(\vec f^{m+1}, \vec U^{m+1} \right)\,.
\end{align}
...
+\tau\,\sum_{k=0}^{m}\left(\vec f^{k+1}, \vec U^{k+1} \right)\,,
\end{align}
for $m=0,\ldots, M-1$.
\end{theorem}
\begin{proof}
...
\right\rangle_{\Gamma^m} \\
& \qquad = \tau\left(\vec f^{m+1}, \vec U^{m+1} \right)\,,
\end{align*}
...
in your paper, why? shouldn't be $I_0^k\rho^{k-1}$}
...
\vec c_i := &\left( \frac{I^m_0\rho^{m-1}+\rho^m}{2\tau_m}\vec I_2^m
...
We observe that the source term $\vec f$ might depend on the fluid density
$\rho$. Therefore, it can be decomposed as $\vec f = \rho\,\vec f_1+\vec f_2$.
At discrete level, we let the density lag one step behind leading to
$\vec f^{m+1} = \vec \rho^m\,f^{m+1}_1+\vec f^m_2$.
