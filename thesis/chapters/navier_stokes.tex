...
\subsection{Existence and uniqueness of a discrete solution}\label{eq:ns_existence}
...
proof of Theorem~\ref{thm:ex}.
%Given that $\vec U \in RM$,
...
%uniform $\tau$?}
% WELL SPOTTED!
% THE UNIFORM TIME STEP SIZE IS TO ENSURE THAT THE DISCRETE TIME DERIVATIVE
% APPROXIMTION IS CONSISTENT, I.E. THE LIMIT AS \tau \to 0 IS u_t.
...
case for a fixed background mesh, or for adaptive meshes where no coarsening is
employed during the evolution. See \cite{fluidfbp} for details.
...
time level $m$, is a fixed point iteration that consists of solving a coupled
