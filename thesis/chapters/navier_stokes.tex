...
%This chapter is extensively based on a paper in preparation.
...
%,i.e. the limit as $\tau \to 0$ is $\vec u_t$. 
Moreover, let ${\cal T}^m$,
...
P1$^{\mbox{\footnotesize face bubble}}$--P0. See \S\ref{sec:stokes_fem} for more details.
...
case for (a) 
a fixed background mesh, or for (b) adaptive meshes where no coarsening is
...
and $(\vec U^{m+1}, P^{m+1}, \kappa^{m+1}) = 
(\vec U^{m+1,s+1}, P^{m+1,s+1}, \kappa^{m+1,s+1})$. 
The scheme (\ref{eq:ns_HGfixedpointa}--d), at each
...
\vec X^{m+1,s+1}, \kappa^{m+1,s+1})$ at each step $s$ until the $L^\infty$--error of the
