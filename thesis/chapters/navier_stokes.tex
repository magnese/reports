...
parametric finite elements to describe the moving discrete interface and
employs standard velocity and pressure finite element spaces in the bulk. This
chapter is extensively based on a paper in preparation.
...
discretization; in \S\ref{sec:ns_existence} we prove the
existence and uniqueness of the antisymmetric discrete approximation; in
\S\ref{sec:ns_stability} we demonstrate that our antisymmetric scheme is
unconditionally stable under certain conditions; in \S\ref{sec:ns_fem} we
present our standard fitted finite element discretization; in
\S\ref{sec:ns_solution_method} we explain the various solutions methods used to
...
Next, we can use Reynolds transport theorem, see \cite{Aris89}, together with
...
P1$^{\mbox{face bubble}}$--P0. See \S\ref{sec:stokes_fem} for more details.
...
\section{Existence and uniqueness of a discrete solution}\label{eq:ns_existence}
...
\end{equation}
which implies that $\mat D(\vec U) = \mat 0$. Given that $\vec U \in RM$,
where $RM$ is the space of rigid motions (\ref{eq:rigid_motions_space}) which
constitutes the kernel of $\mat D$, in particular we have that $\vec U(\vec z)$
is linear in $\vec z$. Moreover, $\vec U\in\uspacedisc{0}{m}$ means that $\vec
U(\vec z)=\vec 0$ for every corner $\vec z$ of $\Omega$. Therefore $\vec U(\vec
z)=\vec 0$ for all $\vec z\in\Omega$. In addition, it holds that $\vec X$ is
equal to a constant on $\Gamma^m$, which satisfies, on recalling
(\ref{eq:ns_proofc}) and $\vec U = \vec 0$, that
\begin{equation} \label{eq:ns_Xconst}
\left\langle \vec X, \chi\,\vec\nu^m \right\rangle_{\Gamma^m}^h = 0
\quad\forall\ \chi \in \Wh\,.
\end{equation}
It was shown in \cite[Proof of Theorem~2.1]{gflows3d} that if $\Gamma^m$
has no self-intersections, then (\ref{eq:ns_Xconst}) immediately yields that
$\vec X = \vec 0$. As $\Gamma^m = \partial\Omega^m_-$ is the boundary of an open
domain, we always assume that it does not self-intersect, and
hence we obtain that $\vec X = \vec 0$. This means that (\ref{eq:ns_proofd})
reduces to
\begin{equation} \label{eq:ns_kappaproof}
...
Then it follows from choosing $\vec\eta = \vec\omega^m$ in
(\ref{eq:ns_kappaproof}) that
\begin{align*}
...
\end{align*}
and so $\kappa = 0 \in \Wh$. Finally, it follows from (\ref{eq:ns_proofa}) with
...
\section{Stability}\label{sec:ns_stability}
We now demonstrate that the scheme (\ref{eq:ns_HGa_antisym}--d) is
unconditional stable. The proof is similar to the Stokes case presented in
\S\ref{sec:stokes_stability} and follow closely the paper \cite{fluidfbp}.
\footnote{Robert: \ref{eq:ns_stabstab} the convective term has $\rho^{k-1}$
in your paper, why? shouldn't be $I_0^k\rho^{k-1}$. Moreover, why do we need an
uniform $\tau$?}
...
\nonumber \\
& \qquad + \tfrac{1}{2}\left((I^m_0\,\rho^{m-1})\,(\vec U^{m+1}
...
\end{equation}
for $m=1,\ldots, M-1$. Then it holds that
...
(\ref{eq:delta_length_inequality}). The desired result (\ref{eq:ns_stabstab})
immediately follows from (\ref{eq:ns_stab}) and (\ref{eq:ns_stabstab_hp}).
\end{proof}

The assumption (\ref{eq:ns_stabstab_hp}) in Theorem~\ref{thm:ns_stability} is
for instance satisfied when either $\mathcal{T}^m = \mathcal{T}^0$ for
$m=1,\ldots, M-1$ or when $\uspace{0}^{m-1} \subset \uspace{0}^m$ for
$m=1,\ldots, M-1$. In the context of unfitted approach,
\S\ref{sec:stokes_fitted_unfitted}, it is equivalent of imposing that no bulk
mesh adaptation is employed, first case, or that bulk mesh refinement routines
without coarsening are used, second case. See \cite{fluidfbp} for more details.
Instead, for a fitted scheme, the first condition means that the bulk mesh
needs to be always equal to the initial mesh while the second condition is
violated whenever the velocity is interpolated on a bulk mesh which does not
contain all the vertices of the previous bulk mesh. Therefore, with a fitted
approach, a part for a stationary problem where the bulk mesh remains fixed,
the scheme does not satisfy the unconditionally stability bound. Indeed, also
without remeshing and smoothing, the velocity needs to be interpolated if the
interface displacement is not null. \footnote{Robert: in the fitted case we
always interpolate the velocity also without smoothing and remeshing since the
interface moves and consequently the bulk moves. Therefore, the condition
\ref{eq:ns_stabstab_hp} is never satisfied! So probably this unconditional
stability part needs to go.}
...
\end{align}
\end{subequations}
and increment $s$, until $\|\vec U^{m+1,s+1}-\vec U^{m+1,s}\|_{L^\infty}
\leq\epsilon_f$. Finally, set ${\Gamma^{m+1}=\vec X^{m+1}(\Gamma^m)}$ and $\vec
U^{m+1}= \vec U^{m+1,s+1}$. The scheme (\ref{eq:ns_HGfixedpointa}--d), at each
...
standard scheme (\ref{eq:ns_HGa}--d). In practice, we also require $s>1$, which
means that we do at least two full iterations of the fixed point scheme. Again,
at every step, the linear algebraic system arising from
(\ref{eq:ns_HGfixedpointa}--d) is identical to the two-phase Stokes algebraic
system (\ref{eq:stokes_algebraic}) with the only modification in the terms
$\vec B_{\Omega}$ and $\vec c$ which become
\begin{align*}
[\vec B_\Omega]_{ij} := & \left( \frac{\rho^m}{\tau_m} \phi_j^{\uspacesimple^m},
\phi_i^{\uspacesimple^m} \right)\mat \id
+ 2\left(\left(\mu^m\,\mat D(\phi_j^{\uspacesimple^m} \vec e_q),
