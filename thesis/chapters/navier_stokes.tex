...
\S\ref{sec:ns_numerical_results}, we also allow for non divergence-free
problem where $\nabla\,.\,\vec u = f_{\rm{div}}$ in $\Omega_\pm(t)$. Therefore
(\ref{eq:advect}) is no longer valid and we need to use
(\ref{eq:fulladvect}) to derive the antisymmetric weak formulation which leads
to the additional term $\tfrac{1}{2}(\rho\,f_{\rm{div}}\,\vec u,\vec\xi)$ in the
momentum equation (\ref{eq:ns_weaka_antisym}). Consequently
(\ref{eq:ns_weaka_antisym}--d) is replaced by
\begin{subequations}
\begin{align}
& \tfrac{1}{2}\bigg[ \ddt (\rho\,\vec u, \vec \xi) + (\rho\,\vec u_t, \vec \xi)
+ (\rho, [(\vec u\,.\,\nabla)\,\vec u]\,.\,\vec \xi
