...
\end{equation}
where $\nabla_s \,.\, \vec g = \sum_{j = 1}^{d} \underline{D}_j g_j$
denotes the surface divergence of a vector field $\vec g$. In particular, if
$\Gamma = \mathcal{S}^{d - 1}$ and the unit normal field is chosen to point away
...
By choosing again the particular function $f(\vec z) = z_j$, $j \in \{ 1,
\hdots, d \}$ in (\ref{LaplaceBeltrami}) and recalling the application of the
...
\nu_j\, ,
\end{equation}
\begin{equation}\label{BeltramiXHnu}
\Delta_s \vec z = \varkappa \vec \nu \qquad \mbox{ on } \Gamma \, .
\end{equation}
This identity of differential geometry will be useful at a later stage
for the presentation of finite element approximations for
\textit{surface diffusion}, see Section \ref{numApproxGeomFlows}.
