...
\subsection[Mean curvature flow with sphere as initial condition]{Mean curvature
flow with sphere as initial condition}
...
sphere. Let $\Gamma(t) = \partial B_{r(t)}(\vec z_0) \subset \R^d$,
oriented by the outer normal
$\vec \nu(\vec{z}) = \frac{\vec z - \vec z_0}{r(t)}$.
...
variable ODE is easily given by
$r(t) = \sqrt{r_0^2 - 2(d - 1)t}, \, 0 \leq t < t_e$, where $\Gamma_0 =
\partial B_{r_0}(\vec z_0)$ and $t_e=\frac{r_0^2}{2(d - 1)}$ is the so called
extinction time. It is worth noting that the surface $\Gamma(t)$ shrinks to a
point as $t \rightarrow t_e$.
...
extinction time, is $t_e=\frac{25}{4}=6.25$. The number of mesh elements is
74770 and there are 37387 nodes. The time step is constant and equal to
$\tau=0.1$.
