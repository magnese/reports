...
$\Gamma(0) = \Gamma_0$ and $\vec u(0) = \vec u_0$, for almost all $t\in(0,T)$
find $\Gamma(t)$ and ${(\vec u, p, \varkappa)}$ ${\in \uspace g \times
\pnormspace \times H^1(\Gamma(t))}$ such that
\begin{subequations}
\begin{align}
...
given as follows. Given $\Gamma(0) = \Gamma_0$ and $\vec u(0) = \vec u_0$, for
...
\pnormspace$. Then, for $d=2$, possible pairs $(\uspacedisc{0}{m},\pspace^m)$
that satisfy the LBB inf-sup condition (\ref{eq:LBB}) are P2--P0 and
P2--(P1+P0), i.e. we set $\uspacedisc{0}{m}=[S^m_2]^d\cap\uspace{0}$ and either
$\pspace^m = S^m_0$ or $S^m_1+S^m_0$, while for $d=3$ are the P3--(P2+P0)
element and P1$^{\mbox{\footnotesize face bubble}}$--P0. See
...
the centre of mass of unequal masses placed at its vertices.
...
fictitious background lattice that covers the domain $\overline{\Omega}$. Then,
at each element of $\mathcal{T}^m$ is assigned the point of the lattice which is
closest to its barycentre. Depending on the size of the lattice, multiple
