...
material properties and pressure at the interface, in order to suppress 
...
negative where $\Omega_-(t)$ is locally convex. In particular, see e.g.\ 
\cite{DeckelnickDE05}, it holds that
...
Mutiplying (\ref{eq:LBop}) with a test function, and performing integration by 
parts, yields
$$ 
...
Here we have observed that if $p \in \pspace$ is part of a solution to 
(\ref{eq:full_momentum}--g), then so is $p + c$ for an arbitrary $c\in \R$.
...
$\Omega$, see \cite[p.~114]{GiraultR86}. In our simulations, we use the 
...
64 & *3.66077e-04 & *1.92562e-04 & *2.87838e-03 & *62110\\
\hline
\end{tabular}
