...
\begin{frame}
\frametitle{Navier--Stokes weak formulation}
...
\begin{align*}
& \blue \left( \rho^m\,\frac{\vec U^{m+1} - \,\vec I^m_2\,\vec
U^m}{\tau^m},\vec \xi \right) \notag\\
& \blue + \frac{1}{2}\left(\rho^m, [(\vec I^m_2\,\vec U^m\,.\,\nabla)\,\vec
U^{m+1}]\,.\,\vec \xi - [(\vec I^m_2\,\vec U^m\,.\,\nabla)\,\vec \xi]\,
.\,\vec U^{m+1} \right)\notag \\
& \black + 2\left(\mu^m\,\mat D(\vec U^{m+1}), \mat D(\vec \xi) \right)
- \left(P^{m+1}, \nabla\,.\,\vec \xi\right) - \gamma\,\left\langle
\kappa^{m+1}\,\vec\nu^m, \vec\xi\right\rangle_{\Gamma^m} \notag \\
& = \left(\rho^m\,\vec f^{m+1}_1 + \vec f^{m+1}_2, \vec \xi\right)
\qquad \forall\ \vec\xi \in \uspace^m \,, \\
& \left(\nabla\,.\,\vec U^{m+1}, \varphi\right)  = 0
\quad \forall\ \varphi \in \widehat\pspace^m\,,\\
&  \left\langle \frac{\vec X^{m+1} - \vec \id}{\tau} ,
\chi\,\vec\nu^m \right\rangle_{\Gamma^m}^h
- \left\langle \vec U^{m+1},
