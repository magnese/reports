...
by setting
\footnote{Robert: ''Marco: I made these changes. OK? '' You simply state the
explicit time dependency of the basis function in the reference domain, right?!
Fine for me.}
\begin{align*}
\uspaceale b &:= \{ \vec\phi:
...
P2--P0 \footnote{Robert: Why P2--P0? We are not using this pace.} and
P2--(P1+P0), i.e.\ we set
...
\footnote{Robert: ''Marco: The spaces $\uspacesemidiscref{g}{h}$ and
$\pnormspaceref^h$ are not defined here? I have also corrected some mistakes
below, not sure if this is still in time for your thesis.'' Yes you are right,
the definitions are missing. What did you correct? I have noticed the change
from q to p in the domain velocity but I don't see anything else. }
\begin{align*}
...
\footnote{Robert: ''Marco: State that $\W^{m+1}$ is given data? What space is
it in? Should we not also say how $\Omega^{m+1}$ is obtained? Should we say
here how $\W^{m+1}$ will be chosen in practice?
I.e. not given data, but determined implicitly via mesh smoothing etc? '' I
would put everything in the ALE solution method section. We can simply say
there that we chose as domain velocity the displacement computed by the
smoothing routine. Then the space is already defined and also $\Omega^{m+1}$.}
...
\footnote{Robert: ''Marco: Why not for the antisymmetric scheme? Should mention
that.'' All the simulations crashed with the antisymmetric scheme in this case
(probably due to elements overlapping).}
