...
\footnote{Marco: Add definitions for $\uspacesemidiscref{g}{h}$ and
$\pnormspaceref^h$}
\begin{align} \label{eq:upalespace}
...
S^h_0(t)$.
\footnote{Robert: Wrong space! Sort out by changing symbols for spaces.}
...
$\Omega^m=\vec x^m(\D)$, $m=1,\ldots,M$, where $\vec x^m \in [S^h_1]^d$
...
Firstly, and similarly to (\ref{eq:upalespace}), 
let $\uspacediscale{g}{m}= \{ \vec\phi:
\Omega^m \to \R^d,\
\vec \phi=\vec{\hat\phi}\circ(\vec x^m)^{\,-1},\ \vec{\hat\phi}$
and $\pnormspaceale^m=
\{ \eta: \Omega^m\to \R,\
\eta=\hat\eta\circ(\vec x^m)^{\,-1},\ \hat\eta\in\pnormspaceref^h \}$.
...
\footnote{Robert: We should say how $\Omega^{m+1}$ is obtained. Come back to
this. Maybe seek $\W^{m+1}$, with right trace on $\Gamma^m$, and then
define $\vec x^{m+1} = \vec x^m + \W^{m+1}$.}
...
for the definitions of the various benchmark quantities.
\begin{table*}
\center
\begin{tabular}{rrrrrr}
\hline
...
Moreover, we notice that for the ALE scheme $(\schemeALE)$, 
the number of remeshings performed during the simulation for this test case
is always between 2 and 4.
...
{($\rho_+ = 10^3,\rho_- = 10^2,\mu_+ = 10,\mu_- =1,\gamma = 24.5$)
Pressure evolution for the rising bubble experiment I for the scheme
$(\schemeAim)$,
...
$(\schemeAex)$, $(\schemeAim)$ and $(\schemeALE)$.
\begin{figure}[htbp]
\centering
...
definitions of the various benchmark quantities. 
We use $J_\Gamma=128$ interface elements
