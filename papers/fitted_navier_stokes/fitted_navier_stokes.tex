...
\subsection{Alternative non symmetric weak formulation}
...
completeness we report the derivation of that non symmetric weak formulation.
...
Hence an alternative non symmetric weak formulation of
(\ref{eq:full_momentum}--g) is given as follows. Given $\Gamma(0) = \Gamma_0$,
for almost all $t\in(0,T)$ find $\Gamma(t)$ and ${(\vec u, p, \varkappa)}$ ${\in
\uspace \times \widehat\pspace \times H^1(\Gamma(t))}$ such that
\begin{subequations}
...
(\ref{eq:fulladvect}) to derive the non symmetric weak formulation.
