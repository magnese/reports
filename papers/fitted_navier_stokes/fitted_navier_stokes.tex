...
% Let $\vec{\mathcal{A}}_t$ be an homeomorphic map which at each time $t\in
% [0,T]$ associate a point $\vec Y$ of a reference domain $\Omega_0$ to a point
% $\vec x$ in the current domain configuration $\Omega_t$:
% \begin{equation}
% \vec{\mathcal{A}}_t:\Omega_0 \subset \R^d\rightarrow \Omega_t \subset \R^d,
% \qquad \vec x(\vec Y,t)=\vec{\mathcal{A}}_t(\vec Y).
% \end{equation}
% We assume that the application
% \begin{equation}
% t\rightarrow \vec x(\vec Y,t),\qquad \vec Y \in\Omega_0
% \end{equation}
...
% Finally, we indicate with $\left.\frac{\partial h}{\partial t}\right|_{\vec x}$
% the partial time derivative in the Eulerian frame and with
% \begin{equation}
% \left.\frac{\partial h}{\partial t}\right|_{\vec Y}:\Omega_t\times
% [0,T]\rightarrow \mathbb{R},\qquad\left.\frac{\partial h}{\partial
% t}\right|_{\vec Y}=h(\vec{\mathcal{A}}_t(\vec Y),t).
% \end{equation}
...
% Then we have for any $\vec v \in \uspace$
% that
...
% (\ref{eq:advect}) below.
%
% Hence, it follows from (\ref{eq:full_momentum},\ref{eq:full_mass}),
% (\ref{eq:tripleterm}) and (\ref{eq:ibp0}) that
% \begin{align}\label{eq:advect}
% & ( \rho\,((\vec u - \vec w) \,.\,\nabla)\,\vec u, \vec \xi)
%  = \tfrac{1}{2} [ (\rho\,((\vec u - \vec w)\,.\,\nabla)\,\vec u, \vec \xi) -
% (\rho\,((\vec u - \vec w)\,.\,\nabla)\,\vec \xi,\vec u) \nonumber \\
% & -\langle [\rho]_-^+(\vec u - \vec w)\,.\,\vec \nu, \vec u\,.\,\vec \xi
% \rangle_{\Gamma(t)} -(\rho\,(\nabla\,.\,\vec w ),\vec u\,.\,\vec\xi) ] \qquad
...
% \qquad \forall
% \vec \xi \in H^1(\Omega, {\mathbb R}^d)
% \,.
% \label{eq:rhot1}
% \end{align}
...
% & (\rho\,[\vec u_t + ((\vec u - \vec w)\,.\,\nabla)\,\vec u], \vec \xi)
% = \tfrac{1}{2}[ \ddt (\rho\,\vec u, \vec \xi) + (\rho\,\vec u_t, \vec \xi)
% \nonumber \\
% & + (\rho, [((\vec u - \vec w)\,.\,\nabla)\,\vec u]\,.\,\vec \xi
% - [((\vec u - \vec w)\,.\,\nabla)\,\vec \xi]\,.\,\vec u)
% -\langle [\rho]_-^+\vec w\,.\,\vec \nu, \vec u\,.\,\vec \xi
% \rangle_{\Gamma(t)} -(\rho\,(\nabla\,.\,\vec w ),\vec u\,.\,\vec\xi) ].
% \end{align}
...
% We note that the term $\langle [\rho]_-^+\vec w\,.\,\vec \nu, \vec u\,.\,\vec
% \xi \rangle_{\Gamma(t)}$ in (\ref{eq:weaka}) is null since, by
% construction, $\vec w = 0$ on $\Gamma (t)$.

We note that we are using a different, and more standard, weak formulation
respect to paper \cite{fluidfbp}. Indeed, there the authors use
\cite[eq. 3.7]{fluidfbp} in order to obtain a weak formulation for which it is
easy to derive an energy bound not only on the continuous level but also on
discrete level, as well as existence/uniqueness on discrete level.
...
(\ref{eq:full_momentum})-(\ref{eq:full_initial_interface}) with
(\ref{eq:full_velocity}) substitute by $- \nabla\,.\,\vec u = -\big(
\alpha_2\charfcn{\Omega_+(t)}\big(r(t)^2-\frac{2+d}{d}
|\vec z|^2\big)-\alpha_1\big)d$ is
...
r(t)^2\big)-8\alpha_2\frac{\mu_+}{\rho}\bigg)\bigg)\vec z
\end{align}
