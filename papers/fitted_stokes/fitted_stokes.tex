% LAST CHANGES BY MA 28/01/16
% Submitted to Int. J. Numer. Methods Fluids on 26/10/15. (ID: FLD-15-0367)
% Received referee reports on 26/12/15.
% Resubmitted to Int. J. Numer. Methods Fluids on 28/01/16.
...
onto $\Vh$, i.e.\ \linebreak $\left\langle \vec\omega^m, \vec\varphi
...
\widehat\pspace^m \times \Vh \times \Wh$ be a solution to
...
Next we can prove that discrete stationary solution exist for our scheme.
\begin{thm} \label{thm:stat2}
Let $(\uspace^m,\pspace^m)$ satisfy the LBB condition \mbox{\rm (\ref{eq:LBB})}
and let $S^m_0 \subset \pspace^m$. Let $\vec f^{m+1} = \vec 0$. Moreover, let
...
\end{equation}
is the unique solution to \mbox{\rm (\ref{eq:HGa}--d)}.
\end{thm}
\begin{proof}
...
(0,T]$ find $\Gamma^h(t)$ and $(\vec U^h(t), P^h(t), \vec{\mathcal{V}}^h(t),
\kappa^h(t)) \in \uspace^h(t) \times \widehat\pspace^h(t) \times \Vht \times
\Wht$ such that
\begin{subequations}
\begin{align}
...
(\ref{eq:lin}) directly follow from (\ref{eq:HGa}--d).
...
For the solution of (\ref{eq:lin}) we use a Schur complement approach
...
stationary spherical bubble in 3d. Let $\mu = \gamma = 1$. Then the true
solution (\ref{eq:radialr},b) reduces to
...
(\ref{eq:conformal}). We report the errors for the P2--P0 element in
Table~\ref{tab:bubble3Dp2p0}, while Table~\ref{tab:bubble3Dp2p1p0} shows the
same errors for the P2--(P1+P0) element. Here we note that, as yet, for
the pairs P2--P0 and P2--(P1+P0) there exist no proofs in the literature that
the LBB condition (\ref{eq:LBB}) holds, see e.g.\ the discussion in
\cite[Remark~8.4.3]{BoffiBF13}. However, in practice we encountered no problems
when using these spaces, and our iterative solver always converged to a
solution of the scheme (\ref{eq:HGa}--d).
For both sets of simulations we use the uniform time step size
$\tau = 10^{-2}$.
\begin{table*}
\center
\begin{tabular}{rllllr}
